...
async function getCloudinarySignature(mode, payload) {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(text || 'Signature function failed');
        }
        return await response.json();
    } catch (e) {
        console.error('Error getting Cloudinary signature:', e);
        throw e;
    }
}
...
async function uploadImageToCloudinary(file, productId = null) {
    try {
        ...
        // إضافة التوقيع للـ signed upload (من Netlify Function)
        const folderToUse = productId ? `${cloudinaryConfig.folder}/${productId}` : cloudinaryConfig.folder;
        const sig = await getCloudinarySignature('upload', {
            folder: folderToUse,
            upload_preset: cloudinaryConfig.uploadPreset
        });
        if (sig && sig.signature && sig.timestamp) {
        ...
        console.log('إعدادات الرفع:', {
            ...
            folder: folderToUse,
            signature: (sig && sig.signature) ? 'generated' : 'missing'
        });
...
async function deleteImageFromCloudinary(publicId) {
    try {
        ...
        // إضافة التوقيع للـ signed delete (من Netlify Function)
        const sig = await getCloudinarySignature('destroy', { public_id: publicId });
        const timestamp = sig && sig.timestamp ? sig.timestamp : Math.floor(Date.now() / 1000);
