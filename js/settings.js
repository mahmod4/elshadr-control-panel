    ...
    try {
        ...
        pageContent.innerHTML = `
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="card">
                    <h2 class="text-2xl font-bold mb-4">بيانات العميل عند الطلب</h2>
                    <form id="customerFieldsForm" onsubmit="saveCustomerFieldsSettings(event)">
                        <div class="form-group">
                            <label>حقول بيانات العميل</label>
                            <div id="customerFieldsEditor" class="border rounded-lg p-3 bg-gray-50">
                                <div id="customerFieldsRows" class="flex flex-col gap-3">
                                    ${(Array.isArray(settings.customerFields) ? settings.customerFields : []).map((f, idx) => `
                                        <div class="grid grid-cols-1 md:grid-cols-6 gap-2 items-center border rounded p-2 bg-white" data-customer-field-row="1">
                                            <div class="md:col-span-2">
                                                <label class="text-sm text-gray-600">اسم الحقل (يظهر للعميل)</label>
                                                <input type="text" class="customerFieldLabel" value="${(f && f.label) ? String(f.label).replace(/\"/g, '&quot;') : ''}">
                                            </div>
                                            <div>
                                                <label class="text-sm text-gray-600">النوع</label>
                                                <select class="customerFieldType">
                                                    <option value="text" ${f && f.type === 'text' ? 'selected' : ''}>نص</option>
                                                    <option value="tel" ${f && f.type === 'tel' ? 'selected' : ''}>هاتف</option>
                                                    <option value="textarea" ${f && f.type === 'textarea' ? 'selected' : ''}>مربع نص</option>
                                                </select>
                                            </div>
                                            <div>
                                                <label class="text-sm text-gray-600">إلزامي</label>
                                                <select class="customerFieldRequired">
                                                    <option value="true" ${f && f.required ? 'selected' : ''}>نعم</option>
                                                    <option value="false" ${!f || !f.required ? 'selected' : ''}>لا</option>
                                                </select>
                                            </div>
                                            <div class="md:col-span-2">
                                                <label class="text-sm text-gray-600">قيمة افتراضية</label>
                                                <input type="text" class="customerFieldDefaultValue" value="${(f && f.defaultValue) ? String(f.defaultValue).replace(/\"/g, '&quot;') : ''}">
                                            </div>
                                            <div class="md:col-span-6 flex justify-end">
                                                <button type="button" class="btn-danger" onclick="removeCustomerFieldRow(this)">حذف الحقل</button>
                                            </div>
                                        </div>
                                    `).join('')}
                                </div>
                                <button type="button" class="btn-primary w-full mt-3" onclick="addCustomerFieldRow()">
                                    <i class="fas fa-plus ml-2"></i>إضافة حقل
                                </button>
                                <small class="text-gray-500 block mt-2">سيتم عرض هذه الحقول في صفحة السلة داخل المتجر ويمكنك إضافة/حذف أي حقل.</small>
                            </div>
                        </div>

                        <button type="submit" class="btn-primary w-full">
                            <i class="fas fa-save ml-2"></i>حفظ بيانات العميل
                        </button>
                    </form>
                </div>
...
window.addCustomerFieldRow = function () {
    const rows = document.getElementById('customerFieldsRows');
    if (!rows) return;
    const div = document.createElement('div');
    div.className = 'grid grid-cols-1 md:grid-cols-6 gap-2 items-center border rounded p-2 bg-white';
    div.setAttribute('data-customer-field-row', '1');
    div.innerHTML = `
        <div class="md:col-span-2">
            <label class="text-sm text-gray-600">اسم الحقل (يظهر للعميل)</label>
            <input type="text" class="customerFieldLabel" value="">
        </div>
        <div>
            <label class="text-sm text-gray-600">النوع</label>
            <select class="customerFieldType">
        ...
        <div class="md:col-span-2">
            <label class="text-sm text-gray-600">قيمة افتراضية</label>
            <input type="text" class="customerFieldDefaultValue" value="">
        </div>
        <div class="md:col-span-6 flex justify-end">
            <button type="button" class="btn-danger" onclick="removeCustomerFieldRow(this)">حذف الحقل</button>
        </div>
...
window.removeCustomerFieldRow = function (btn) {
    try {
        const row = btn && btn.closest ? btn.closest('[data-customer-field-row]') : null;
...
window.collectCustomerFieldsFromForm = function () {
    ...
    const result = [];
    rows.forEach((row) => {
        const labelEl = row.querySelector('.customerFieldLabel');
        const typeEl = row.querySelector('.customerFieldType');
        const reqEl = row.querySelector('.customerFieldRequired');
        const defEl = row.querySelector('.customerFieldDefaultValue');
        const label = labelEl && typeof labelEl.value === 'string' ? labelEl.value.trim() : '';
        const type = typeEl && typeEl.value ? String(typeEl.value) : 'text';
        const required = reqEl && reqEl.value ? reqEl.value === 'true' : true;
        const defaultValue = defEl && typeof defEl.value === 'string' ? defEl.value.trim() : '';
        if (!label) return;
        result.push({ label, type, required, defaultValue });
    });
    return result;
}
...
window.saveCustomerFieldsSettings = async function(event) {
    event.preventDefault();

    const customerFields = collectCustomerFieldsFromForm();
    try {
        const settingsData = {
            customerFields,
            updatedAt: new Date()
        ...
        console.error('Error saving customer fields settings:', error);
